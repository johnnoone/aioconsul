...
Create a session::
    ...
    >>> created = yield from client.session.create(name='foo',
    >>>                                            node='my.node.name',
    >>>                                            ttl='60s')

Fetch this session::

    >>> session = yield from client.session.get(created)
...
List all attached sessions of datacenter, but filtered by my node::
    ...
    >>> sessions = yield from client.session.items(node='my.node.name')

I'm done with it, delete my session::

    >>> deleted = yield from client.session.delete(session)
